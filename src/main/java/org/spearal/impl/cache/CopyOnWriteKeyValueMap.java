...
public final class CopyOnWriteKeyValueMap<K, V> {
	...
	public CopyOnWriteKeyValueMap(boolean useIdentity, ValueProvider<K, V> provider) {
		if (useIdentity)
			 this.map = new IdentityValueMap<K, V>(1, provider);
		else
			 this.map = new EqualityValueMap<K, V>(1, provider);
	}
	...
	public synchronized V putIfAbsent(SpearalContext context, K key) {
		KeyValueMap<K, V> clone = get().clone();
		V value = clone.putIfAbsent(context, key);
		set(clone);
		return value;
	}

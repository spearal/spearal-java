 ...
 * @author Franck WOLFF
 */
public final class StringCache<T> {
	...
	public StringCache(ValueProvider<T> provider) {
		this.provider = provider;

		init(DEFAULT_INITIAL_CAPACITY);
	}
	
	public StringCache(ValueProvider<T> provider, int capacity) {
		this.provider = provider;
	...
	public T putIfAbsent(String key) {
	...
	private final static class Entry {
		...
		public Entry(String key, int hash, Object value, Entry next) {
			this.key = key;
			this.hash = hash;
			this.value = value;
			this.next = next;

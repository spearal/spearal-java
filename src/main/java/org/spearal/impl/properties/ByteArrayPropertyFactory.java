 ...
 * @author Franck WOLFF
 */
public class ByteArrayPropertyFactory implements PropertyFactory, ObjectWriterProvider {
	
	@Override
	public ObjectWriter getWriter(Class<?> type) {
		return (ByteArrayProperty.canCreateWriter(type) ? ByteArrayProperty.createWriter() : null);
	}

	@Override
	public boolean canCreateProperty(Class<?> type) {
		return ByteArrayProperty.canCreateProperty(type);
	}

	@Override
	public Property createProperty(String name, Field field, Method getter, Method setter) {
		return new ByteArrayProperty(name, field, getter, setter);
	}
}

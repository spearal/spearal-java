 ...
 * @author Franck WOLFF
 */
public class ClassPropertyFactory implements PropertyFactory, ObjectWriterProvider {

	@Override
	public ObjectWriter getWriter(Class<?> type) {
		return (ClassProperty.canCreateWriter(type) ? ClassProperty.createWriter() : null);
	}

	@Override
	public boolean canCreateProperty(Class<?> type) {
		return ClassProperty.canCreateProperty(type);
	}

	@Override
	public Property createProperty(String name, Field field, Method getter, Method setter) {
		return new ClassProperty(name, field, getter, setter);
	}
}

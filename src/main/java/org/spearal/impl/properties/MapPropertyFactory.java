 ...
 * @author Franck WOLFF
 */
public class MapPropertyFactory implements PropertyFactory, ObjectWriterProvider {
	
	@Override
	public ObjectWriter getWriter(Class<?> type) {
		return (MapProperty.canCreateWriter(type) ? MapProperty.createWriter() : null);
	}

	@Override
	public boolean canCreateProperty(Class<?> type) {
		return MapProperty.canCreateProperty(type);
	}

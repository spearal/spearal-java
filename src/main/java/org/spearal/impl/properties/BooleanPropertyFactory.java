 ...
 * @author Franck WOLFF
 */
public class BooleanPropertyFactory implements PropertyFactory, ObjectWriterProvider {
	
	@Override
	public ObjectWriter getWriter(Class<?> type) {
		return (BooleanProperty.canCreateWriter(type) ? BooleanProperty.createWriter() : null);
	}

	@Override
	public boolean canCreateProperty(Class<?> type) {
		return BooleanProperty.canCreateProperty(type);
	}

	@Override
	public Property createProperty(String name, Field field, Method getter, Method setter) {
		return new BooleanProperty(name, field, getter, setter);
	}
}
